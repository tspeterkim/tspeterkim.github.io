...
It's not pretty. But it's necessary as we don't have the luxury of `getline`. After parsing each line, we now have a
...
Interestingly, the kernel is slower on a T4 (~8 seconds longer) and so I used `ncu` to profile on both devices.

{% highlight shell %}
> ncu fast data/measurements.txt 1000000 600000
...
